    ...
    Alert.alert(
      ...
      [
        { text: 'Cancelar', style: 'cancel' },
        {
          ...
          onPress: async () => {
            try {
              if (order?.photoUri && Platform.OS !== 'web') {
                try {
                  const { File } = await import('expo-file-system');
                  const file = new File(order.photoUri);
                  if (file.exists) {
                    await file.delete();
                  }
                } catch (e) {
  ...
  return (
    <SafeAreaView style={styles.container}>
      ...
      <View style={styles.header}>
        ...
        <Text style={styles.headerTitle}>Detalhes</Text>
        <Pressable onPress={() => setMenuVisible(true)} hitSlop={12} accessibilityLabel="Menu">
      ...
      <ScrollView contentContainerStyle={styles.scrollContent}>
        ...
        {order?.photoUri ? (
          <Pressable onPress={() => setImageModalVisible(true)}>
            <Image source={{ uri: order.photoUri }} style={styles.heroImage} />
          </Pressable>
        ) : (
              ...
              return (
                <React.Fragment key={s}>
                  ...
                  {idx < STATUS_ORDER.length - 1 && (
                    <View style={[
                      styles.progressLine,
                      { backgroundColor: idx < currentStatusIdx ? color : '#E0E0E0' },
                    ]} />
                  )}
                </React.Fragment>
              );
        ...
        <Pressable style={styles.menuOverlay} onPress={() => setMenuVisible(false)}>
          <View style={styles.menuSheet}>
            <Pressable
              ...
              accessibilityLabel="Editar encomenda"
            >
      ...
      <Modal visible={imageModalVisible} transparent animationType="fade" onRequestClose={() => setImageModalVisible(false)}>
        <View style={styles.imageModalContainer}>
          <Pressable style={styles.imageModalClose} onPress={() => setImageModalVisible(false)} accessibilityLabel="Fechar">
